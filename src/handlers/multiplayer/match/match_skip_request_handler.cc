...
#include "match_skip_request_handler.hh"

void shiro::handler::multiplayer::match::skip_request::handle(shiro::io::osu_packet &in, shiro::io::osu_writer &out, std::shared_ptr<users::user> user) {
    shiro::multiplayer::match_manager::iterate([user](io::layouts::multiplayer_match &match) -> bool {
        ...
        match.skipped_players.at(index) = true;
        ...
        size_t skipped_count = std::count_if(match.skipped_players.begin(), match.skipped_players.end(), [](bool skipped) {
            return skipped;
        });
