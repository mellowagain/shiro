...
#include "room_ready_handler.hh"
#include "../../../multiplayer/match_manager.hh"
#include "../../../utils/slot_status.hh"

void shiro::handler::multiplayer::room::ready::handle_ready(shiro::io::osu_packet &in, shiro::io::osu_writer &out, std::shared_ptr<users::user> user) {
...
void shiro::handler::multiplayer::room::ready::handle_unready(shiro::io::osu_packet &in, shiro::io::osu_writer &out, std::shared_ptr<users::user> user) {
    shiro::multiplayer::match_manager::iterate([user](io::layouts::multiplayer_match &match) -> bool {
        auto iterator = std::find(match.multi_slot_id.begin(), match.multi_slot_id.end(), user->user_id);
