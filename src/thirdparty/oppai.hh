#ifndef SHIRO_OPPAI_HH
#define SHIRO_OPPAI_HH
 ...
 */
OPPAIAPI int ezpp_dup(ezpp_t ez, char const* mapfile);
...
int is_nan(float b) {
    unsigned* p = reinterpret_cast<unsigned*>((void*)&b);
    return (
...
int m_reserve(ezpp_t ez, int min_size) {
    ...
    new_block = reinterpret_cast<char*>(malloc(size));
    if (!new_block) {
        return 0;
    }
    ez->block = new_block;
...
char* m_strndup(ezpp_t ez, char* s, int n) {
    char* res = reinterpret_cast<char*>(m_alloc(ez, n + 1));
    if (res) {
        memcpy(res, s, n);
        res[n] = 0;
    }
...
int p_objects(ezpp_t ez, slice_t* line) {
    ...
    if (ez->mode == MODE_TAIKO) {
        int* sound_type = reinterpret_cast<int*>(m_alloc(ez, sizeof(int)));
        if (!sound_type) {
            return ERR_OOM;
        }
    ...
    if (o->type & OBJ_CIRCLE) {
        ...
        if (ez->mode == MODE_TAIKO && ne > 8 && slice_len(&e[8]) > 0) {
            ...
            o->sound_types = reinterpret_cast<int*>(m_alloc(ez, sizeof(int) * nodes));
            if (!o->sound_types) {
                return ERR_OOM;
            }
...
#define s(x) ez->x = ez->x ? ez->x : nullstr
    s(title);
    s(title_unicode);
    s(artist);
    s(artist_unicode);
...
ezpp_t ezpp_new(void) {
    ezpp_t ez = reinterpret_cast<struct ezpp*>(calloc(sizeof(struct ezpp), 1));
    if (ez) {
        ez->mode = MODE_STD;
        ez->mods = MODS_NOMOD;
        ez->combo = -1;
...
char* strclone(char* s) {
    int len = strlen(s) + 1;
    return reinterpret_cast<char*>(memclone(s, len));
}

char *strclone_const(char const* s) {
    return strdup(s);
}

OPPAIAPI
int ezpp_dup(ezpp_t ez, char const* mapfile) {
    free_owned_map(ez);
    ez->flags |= OWNS_MAP_BIT;
    ez->map = strclone_const(mapfile);
    return calc(ez);
}
...
int ezpp_data_dup(ezpp_t ez, char* data, int data_size) {
    free_owned_map(ez);
    ez->flags |= OWNS_MAP_BIT;
    ez->data = reinterpret_cast<char*>(memclone(data, data_size));
    ez->data_size = data_size;
    return calc(ez);
}
