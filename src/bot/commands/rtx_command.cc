...
bool shiro::bot::commands::rtx(std::deque<std::string> &args, std::shared_ptr<shiro::users::user> user, std::string channel) {
    if (args.empty()) {
        utils::bot::respond("Usage: !rtx <target> [reason]", user, channel);
        return false;
    }
    ...
    if (target == nullptr || !users::manager::is_online(target)) {
        utils::bot::respond(args.at(0) + " is currently not online or does not exist.", user, channel);
        return false;
    }
    ...
    utils::bot::respond(target->presence.username + " has been successfully rtx'ed.", user, channel);
    return true;
}
